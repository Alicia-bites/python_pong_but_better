...
# main loop
while run:
    wn.fill(BLACK)  # to cover past positions of moving objects with BLACK
    # anytime the user press a key or a button,
    # the event associated will be store in pygame.event
    ...
    if ball_y <= 0 + radius or ball_y >= HEIGHT - radius:
        ball_vel_y *= -1  # reverse the y direction of the ball
    if dummy_ball_y <= 0 + radius or dummy_ball_y >= HEIGHT - radius:
        dummy_ball_vel_y *= -1  # reverse the y direction of the ball
    ...
    gad_left_2 = font.render("Gad Left:" + str(left_gadget_remaining), True, WHITE)
    wn.blit(gad_left_2, (825, 65))

    # OBJECTS
    # draw a circle (the ball)
    pygame.draw.circle(wn, BLUE, (ball_x, ball_y), radius)
    # draw rectangles (the paddles)
    ...
    if player_2 >= 3:
        ...
        endscreen = winning_font.render("PLAYER_1 WON!!!", True, WHITE)
        wn.blit(endscreen, (200, 250))
	# take each variable updates into account
